...
{
    public class RoleRemovePermissionOperation : IOperation<RoleRemovePermission>
    {
        private readonly IRoleAggregationStore _aggregationStore;
        private readonly ILogger<RoleRemovePermissionOperation> _logger;

        public RoleRemovePermissionOperation(IRoleAggregationStore aggregationStore, 
            ILogger<RoleRemovePermissionOperation> logger)
        {
            _aggregationStore = aggregationStore ?? throw new ArgumentNullException(nameof(aggregationStore));
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        public async Task<Result> ExecuteAsync(RoleRemovePermission request, CancellationToken cancellationToken = default)
        {
            _logger.LogInformation("Going to remove permission in role. [Role: {roleId}][Permission: {permissionId}]", 
